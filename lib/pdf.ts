...
function renderInvoicePdf(doc: jsPDF, data: DynamicVisitPdfData) {
  ...
  doc.setFillColor("#FFFFFF");
  doc.roundedRect(190, 54, 145, 92, 8, 8, "FD");
  doc.setTextColor(gold);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.text("ISSUED BY", 210, 76);
  doc.setTextColor(ink);
  doc.setFontSize(11);
  doc.text(settings.doctorName, 210, 94, { maxWidth: 105 });
  doc.setTextColor(muted);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text(settings.doctorQualification, 210, 112, { maxWidth: 105 });
  doc.setFont("helvetica", "bold");
  doc.text(`Reg. No.: ${settings.registrationNumber}`, 210, 134, { maxWidth: 105 });
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  doc.text("Payment Status", 350, y + 58);
  doc.setTextColor(gold);
  doc.setFont("helvetica", "bold");
  doc.text(paymentStatus, 504, y + 58, { align: "right" });

  y += 100;
  doc.setDrawColor(line);
  doc.setFillColor("#FFFFFF");
  doc.roundedRect(52, y, 491, 82, 10, 10, "FD");
  doc.setTextColor(ink);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text(`Digitally generated by ${settings.clinicName}`, 72, y + 28, { maxWidth: 300 });
  doc.setTextColor(muted);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text("This invoice is system generated from the saved visit entry.", 72, y + 48, { maxWidth: 300 });
  doc.setDrawColor(gold);
  doc.circle(480, y + 42, 28, "S");
  doc.setTextColor(gold);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.text("CLINIC", 480, y + 38, { align: "center" });
  doc.text("STAMP", 480, y + 50, { align: "center" });
  ...
  doc.setTextColor(ink);
  doc.setFont("helvetica", "bold");
  doc.text(settings.doctorName, 380, 782, { maxWidth: 140 });
  doc.setTextColor(muted);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text(settings.doctorQualification, 380, 798, { maxWidth: 140 });
  doc.text(`Dental Registration No.: ${settings.registrationNumber}`, 380, 812, { maxWidth: 140 });
}
